...
const AboutMeView = () => {
  return (
    <div className=" mx-auto py-14 px-10 md:px-20 rounded-4xl">
      <div className="flex flex-col md:flex-row gap-8 items-center">
        <div className="md:w-1/2 flex justify-center items-center p-4 border-[0.5] border-gray-200 rounded-full min-w-80 min-h-80">
          <Image
            ...
            height={400}
            alt={about.title}
            className="rounded-full object-cover shadow-2xl w-90 h-90"
          />
        </div>
        <div className="md:w-2/3 flex flex-col justify-center items-start gap-3">
          ...
          <Text
            type="xsmall"
            className="md:text-left mb-6 font-semibold font-display tracking-[0.1em]"
          >
            {about.subtitle}
          </Text>
          <div className="mb-8 mx-auto md:mx-0 font-light border-b-[0.5] border-gray-200 pb-8 flex justify-center md:justify-start items-start gap-4 w-full flex-col">
            {about.descriptionItems.map((item, index) => (
              <Text
                type="small"
                className="text-subtle-foreground font-sans"
                key={index}
              >
                <span className="text-green font-medium">•</span> {item}
              </Text>
            ))}
          </div>
          <Text
